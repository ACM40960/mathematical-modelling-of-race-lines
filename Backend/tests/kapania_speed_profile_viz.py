#!/usr/bin/env python3
"""
Kapania speed-profile visualization (4 panels):
 (a) Curvature profile κ(s)
 (b) Zero-longitudinal-force speed (v_ss, "F_x = 0")
 (c) Forward pass result vs v_ss
 (d) Final speed after backward pass vs forward

Outputs: poster_output/kapania_speed_profile.png and .pdf
"""
import os
import numpy as np
...
def compute_profiles(track_points: np.ndarray, car_params: dict, friction: float = 1.0):
    model = KapaniaModel()
    # Extract canonical params used internally (fills defaults)
    ...
    # (b) Steady-state speeds v_ss (Fx = 0 analogue)
    v_ss = np.zeros(n)
    for i in range(n):
        ...
        mass_factor = 798.0 / mass
        v_ss[i] *= (0.90 + 0.10 * mass_factor)
    ...
    for i in range(1, n):
        if distances[i-1] > 0:
            F_lat = mass * (v_forward[i-1] ** 2) * abs(curvature[i-1])
            corner_loss = min(F_lat / (mass * 50.0), 0.3)
            F_available = max_engine_force * (1.0 - corner_loss)
            F_eff = F_available * accel_factor
            v2 = v_forward[i-1] ** 2 + (2 * F_eff * distances[i-1]) / mass
            new_v = np.sqrt(max(v2, min_corner_speed ** 2))
            v_forward[i] = min(new_v, v_ss[i])
    ...
    for i in range(n - 2, -1, -1):
        if distances[i] > 0:
            ...
            F_lat_next = mass * (v_final[i+1] ** 2) * abs(curvature[i+1])
            loss_b = min(F_lat_next / (mass * 30.0), 0.4)
            F_brake_eff = base_brake * mass_factor_b * stab_brake * (1.0 - loss_b)
            v2 = v_final[i+1] ** 2 - (2 * F_brake_eff * distances[i]) / mass
            new_v = np.sqrt(max(v2, min_corner_speed ** 2))
            v_final[i] = min(new_v, v_forward[i])

    # Arc-length s for x-axis
    s = np.cumsum(distances)
    return s, curvature, v_ss, v_forward, v_final


def main():
    # Frontend track data (same as comparison script)
    frontend_track_data = [
    ...
    # Car params: use the same style as comparison
    car_params = {
        'mass': 798,
        'yaw_inertia': 1200,
    ...
    s, kappa, v_ss, v_fwd, v_fin = compute_profiles(track_points, car_params, friction=1.0)
    ...
    # (a) curvature
    axes[0].plot(s, kappa, 'k-', linewidth=1.5)
    axes[0].set_ylabel('κ (1/m)')
    axes[0].grid(True, linestyle=':', linewidth=0.6)

    # (b) v_ss
    axes[1].plot(s, v_ss, 'k-', linewidth=1.5, label=r'$F_x=0$')
    axes[1].set_ylabel('Ux (m/s)')
    axes[1].legend(loc='upper right', frameon=True, fontsize=9)
    axes[1].grid(True, linestyle=':', linewidth=0.6)

    # (c) forward vs v_ss
    axes[2].plot(s, v_fwd, 'k-', linewidth=1.5, label='Forward Pass')
    axes[2].plot(s, v_ss, 'k--', linewidth=1.0, label=r'$F_x=0$')
    axes[2].set_ylabel('Ux (m/s)')
    axes[2].legend(loc='upper right', frameon=True, fontsize=9)
    axes[2].grid(True, linestyle=':', linewidth=0.6)

    # (d) backward vs forward
    axes[3].plot(s, v_fin, 'k-', linewidth=1.5, label='Backward Pass')
    axes[3].plot(s, v_fwd, 'k--', linewidth=1.0, label='Forward Pass')
    axes[3].set_ylabel('Ux (m/s)')
    axes[3].set_xlabel('s (m)')
    axes[3].legend(loc='upper right', frameon=True, fontsize=9)
    axes[3].grid(True, linestyle=':', linewidth=0.6)

    plt.tight_layout()

    out_dir = os.path.join(os.path.dirname(__file__), 'poster_output')
    os.makedirs(out_dir, exist_ok=True)
    png = os.path.join(out_dir, 'kapania_speed_profile.png')
    pdf = os.path.join(out_dir, 'kapania_speed_profile.pdf')
