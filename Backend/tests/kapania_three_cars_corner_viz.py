#!/usr/bin/env python3
"""
Kapania: Corner overlay of three cars on the same track segment.

Shows inner/outer track edges and the three optimized Kapania paths to illustrate
how different parameter sets take the same turn.

Outputs: Backend/tests/poster_output/kapania_three_cars_corner_viz.(png|pdf)
"""
import os
import numpy as np
...
def track_boundaries(points: np.ndarray, half_width: float) -> tuple[np.ndarray, np.ndarray]:
    ...
    outer = points + normals * half_width
    inner = points - normals * half_width
    return outer, inner


def kapania_path(points: np.ndarray, car_params: dict, width: float) -> np.ndarray:
    model = KapaniaModel()
    try:
        path = model.calculate_racing_line(points, compute_curvature(points), width, car_params, friction=1.0)
...
def pick_corner_indices(centerline: np.ndarray, window: int = 60) -> slice:
    kappa = compute_curvature(centerline)
    # Choose the center index with highest curvature (avoid endpoints)
    idx = int(np.argmax(kappa[5:-5]) + 5)
    start = max(0, idx - window // 2)
    end = min(len(centerline), start + window)
    return slice(start, end)


def main():
    # Same frontend path as other tests
    frontend_track_data = [
    ...
    centerline = np.array([[p['x'], p['y']] for p in frontend_track_data])
    if not np.allclose(centerline[0], centerline[-1], atol=1e-3):
    ...
    # Three car setups
    car_setups = [
        ("Aggressive", {'mass': 700, 'yaw_inertia': 1000, 'front_cornering_stiffness': 130000, 'rear_cornering_stiffness': 160000, 'max_engine_force': 17000, 'downforce_factor': 3.5, 'max_straight_speed': 90, 'brake_force_multiplier': 3.2}, '#0066FF'),
        ("Balanced",  {'mass': 798, 'yaw_inertia': 1200, 'front_cornering_stiffness': 100000, 'rear_cornering_stiffness': 140000, 'max_engine_force': 15000, 'downforce_factor': 3.0, 'max_straight_speed': 85, 'brake_force_multiplier': 3.0}, '#FF0000'),
        ("Conservative", {'mass': 900, 'yaw_inertia': 1500, 'front_cornering_stiffness': 80000, 'rear_cornering_stiffness': 110000, 'max_engine_force': 13000, 'downforce_factor': 2.5, 'max_straight_speed': 80, 'brake_force_multiplier': 2.8}, '#00AA00'),
    ]

    # Compute paths
    paths = []
    for label, params, color in car_setups:
        path = kapania_path(centerline, params, track_width)
        paths.append((label, path, color))

    # Boundaries for the centerline (for consistent edges)
    outer, inner = track_boundaries(centerline, half_w)

    # Pick a high-curvature corner slice
    slc = pick_corner_indices(centerline, window=80)
    ...
    # Track edges
    ax.plot(outer[slc, 0], outer[slc, 1], color='black', linewidth=2.0, label='Track Edge')
    ax.plot(inner[slc, 0], inner[slc, 1], color='black', linewidth=2.0)

    # Centerline (optional light)
    ax.plot(centerline[slc, 0], centerline[slc, 1], color='gray', linewidth=1.0, linestyle='--', alpha=0.6)

    # Paths
    for label, path, color in paths:
        ax.plot(path[slc, 0], path[slc, 1], color=color, linewidth=2.5, label=label)
    ...
    png = os.path.join(out_dir, 'kapania_three_cars_corner_viz.png')
    pdf = os.path.join(out_dir, 'kapania_three_cars_corner_viz.pdf')
