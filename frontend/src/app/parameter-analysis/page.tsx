      ...
      <div className="bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-4">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-bold text-gray-900">🔬 Parameter Analysis</h1>
          </div>
          <Link 
            href="/track-designer" 
            ...
            <div className="flex-1 p-4">
              <div className="w-full h-full bg-white rounded-lg shadow-sm border border-gray-200 flex items-center justify-center">
                <div className="text-center max-w-md">
                  ...
                  <h3 className="text-2xl font-bold text-gray-900 mb-4">Parameter Analysis Ready</h3>
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="flex items-center justify-center">
                      <div className={`w-3 h-3 rounded-full mr-3 ${track ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <span className={`text-sm ${track ? 'text-green-600' : 'text-red-600'}`}>
                        Track {track ? 'Available' : 'Required'}
                      </span>
                    </div>
                    <div className="flex items-center justify-center">
                      <div className={`w-3 h-3 rounded-full mr-3 ${cars.length > 0 ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <span className={`text-sm ${cars.length > 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className="w-full bg-gray-50 text-gray-800 text-xs border border-gray-300 rounded shadow-sm">
                ...
                <div className="px-3 py-1.5">
                  ...
                  <div className="space-y-0.5 text-gray-600 text-xs">
                    <div className={`flex items-center ${track ? 'text-green-600' : 'text-red-600'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full mr-2 ${track ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      Track {track ? '✓' : '(create in Track Designer)'}
                    </div>
                    <div className={`flex items-center ${cars.length > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full mr-2 ${cars.length > 0 ? 'bg-green-500' : 'bg-red-500'}`}></div>
                ...
                <div className="px-3 py-1.5">
                  <div className="space-y-1 opacity-50">
                    <label className="flex items-center space-x-1.5">
                      ...
                      <span className="text-gray-500 text-xs">LAP TIME VS PARAMETER</span>
                    </label>
                    <label className="flex items-center space-x-1.5">
                      <input type="checkbox" disabled className="w-2.5 h-2.5" />
                      <span className="text-gray-500 text-xs">PARAMETER VISIBILITY</span>
                    </label>
                  </div>
                </div>
              </div>
